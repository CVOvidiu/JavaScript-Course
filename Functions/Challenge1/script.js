...
poll.registerNewAnswer = function() {
    const opt = Number(prompt(`${this.question}\n${this.options.join('\n')}\n(Write option number)`));

    isNaN(opt) && console.log('You need to introduce a number!');
    
    typeof(opt) === 'number' && opt > this.answers.length && console.log('You need to introduce a valid option!');

    typeof(opt) === 'number' && opt <= this.answers.length && this.answers[opt - 1]++;
